...
def preprocess(list_file_names: List[str], src_data_home:str="clean", dst_data_home:str="processed") -> None:
    """
    ...
    for file in list_file_names:
        src_file_path = os.path.join(data_root, src_data_home, file)
        dst_file_path = os.path.join(data_root, dst_data_home, file)
        with open(src_file_path, "r") as in_file:
            lines = in_file.readlines()
            results = []
            for line in lines:
                results += [l.strip()+".\n" for l in line.strip().split(".") if len(l)>0]
...
def go(config):
    ...
    list_file_names = [file_name.split("/")[-1] for file_name in 
                glob.glob(os.path.join(config["input_path"], "book.txt"))]
    preprocess(list_file_names)

# see main.py for passing arguments value through mlflow run for an entry point in the project. 
